...
class OneStepper(MDP):

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self._job_runner = None
        self._onestep = partial(self.onestep, env=self.env)

    @staticmethod
    def onestep(env, sts, acs, tms, jobid):
        def set_and_step(st, ac, tm):
            env.reset(state=st, tm=tm)
            next_ob, rw, next_dn, _ = env.step(ac)
    ...
    def run(self, sts, acs, tms):
        ...
        if self._n_processes > 1:
            if self._job_runner is None:
                workers = [Worker(method=self._onestep) for _ in range(self._n_processes)]
            ...
            next_obs, rws, next_dns, _ = self._onestep(**kwargs)
